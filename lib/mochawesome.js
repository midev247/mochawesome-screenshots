...
var mocha     = require('mocha-it-steps'),
    _         = require('lodash'),
    uuid      = require('uuid'),
    chalk     = require('chalk'),
...
function Mochawesome (runner, options) {
  ...
  runner.on('pass', function (test) {
    ...
    if (config.passedScreenshot) {
      saveScreenshotToFile(config.reportScreenshotDir + '/' + test.scrFileName);
    }
  });
  ...
  runner.on('end', function () {
    try {
      if (!endCalled) {
        ...
        obj.stats.failures = obj.stats.failures;
...
function cleanSuite (suite) {
  ...
  totalTestsRegistered += suite.title ? 1 : 0;
